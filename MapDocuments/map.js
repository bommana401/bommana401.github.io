...
function initMap() {
  ...
  // document.getElementById('result').value = totalDistance.toFixed(2) + " mi " + "or " + totalDistance.toFixed(3) * 5280 + " ft";
  ...
  document.getElementById("dailyElecConsumption") = 0 + " Wh";
...
function numOfBioDigestors(surfaceArea) {
  //the surface area of panel
  let digestorSA = (83 / 12) * (45 / 12);
  let gasCreated = 700*0.000353147;
  console.log("number of Ccf = ", (gasCreated));

  let numOfDigestors = parseInt(surfaceArea / digestorSA);
  bioHours = numOfDigestors * 2;
  totalDigestors = numOfDigestors;
  console.log("number of biodigestors = ", (numOfDigestors));

  let energy = numOfDigestors * gasCreated;
  return energy;
}
...
function annualElecSavings(){
  ...
  if( elecPower.length == 0 || elecBP.length == 0 || elecCost.length == 0){
    ...
    document.getElementById("dailyElecConsumption").innerHTML = "Your daily electricity consumtion is "+ (elecPower/elecBP)*1000 + " Wh";     // console.log("total solar energy is watts", totalSolarEnergy);
...
function annualGasSavings() {
  ...
  if( gasPower.length >0 && gasBP.length>0 && gasCost.length>0){
    ...
    alert("You need to fill in the textboxes associtate with Gas Power, Cost, or Billing Period");
  }
}
